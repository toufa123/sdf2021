...
get-variant() {
  ...
  if [ -z "$branch" ]; then
    echo >&2 "Could not get the current branch name for commit, not in a branch?: $(git rev-list -n 1 HEAD)"
    return 1
  fi
  case "$branch" in
  master) echo "" ;;
...
tag-and-push() {
  ...
  local target=$2
  local digest_source
  digest_source=$(get-digest ${tag1})
  local digest_target
  digest_target=$(get-digest ${tag2})
